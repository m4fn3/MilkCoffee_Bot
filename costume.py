...
class Costume(commands.Cog):
    ...
    async def add(self, ctx) -> None:
        ...
        if ctx.invoked_subcommand is None:
            user_lang = get_lg(self.bot.database[str(ctx.author.id)]["language"], ctx.guild.region)
            await ctx.send(["サブコマンドが不足しているよ!\n`{}help add`で使い方を確認してね!", "Missing subcommands!\n`{}help add` to see how to use it!", "하위 명령이 부족한거야! \n`{}help add` 사용법을 확인 해!", "¡Faltan subcomandos! \n`{}help add` para ver cómo se usa!"][user_lang].format(self.bot.PREFIX))
    ...
    @add.command(name="body", aliases=["d", "bd", "by"], usage="add body [番号|名称]^add body [number | name]^add body [번호 | 제목]^add body [número | nombre]", description="体装飾を設定できるよ!^Set the body decoration!^몸 장식을 설정 할 수있어!^¡Puedes configurar la decoración del cuerpo!", help="`{}add body 5`...番目の体装飾を設定します\n`{}add body n.s.suit` ... n.s.suitという名前の体装飾を設定します^`{}add body 5` ... sets the third body decoration\n`{} add body n.s.suit` ... Set a body decoration named n.s.suit\n^`{} add body 5` ... 5번째 신체 장식을 설정합니다\n`{} add body n.s.suit` ... n.s.suit라는 신체 코스튬을 설정합니다\n`{}add body 5` ... establece la decoración del tercer cuerpo\n`{}add body n.s.suit` ... Establece la decoración del cuerpo llamada n.s.suit")
    ...
    @add.command(name="back", aliases=["b", "bk", "bc"], usage="add back [番号|名称]^add back [number | name]^add back [번호 | 제목]^add back [número | nombre]", description="背中装飾を指定できるよ!^Set the back decoration!^허리 장식을 지정할 수 있어요!^¡Puedes especificar la decoración de la espalda!", help="`{}add back 6`...6番目の背中装飾を設定します\n`{}add back B.MOUSE` ... B.MOUSEという名前の背中装飾を設定します^`{}add back 6` ... Set the 6th back decoration\n`{}add back B.MOUSE` ... Set the back decoration named B.MOUSE^`{}add back 6` ... 6 번째 등 코스튬을 설정합니다\n`{}add back B.MOUSE` ... B.MOUSE라는 등 코스튬을 설정합니다^`{}add back 6` ... Establecer la sexta decoración trasera\n`{}add back B.MOUSE` ... Establecer la decoración de la espalda llamada B.MOUSE")
    ...
    async def list(self, ctx) -> None:
        ...
        if ctx.invoked_subcommand is None:
            user_lang = get_lg(self.bot.database[str(ctx.author.id)]["language"], ctx.guild.region)
            await ctx.send(["サブコマンドが不足しているよ!\n`{}help list`で使い方を確認してね!", "Missing subcommands!\n`{}help list` to see how to use it!", "하위 명령이 부족한거야! \n`{}help list` 사용법을 확인 해!", "¡Faltan subcomandos! \n`{}help list` para ver cómo se usa!"][user_lang].format(self.bot.PREFIX))
    ...
    async def list_character(self, ctx):
        ...
        if len(listed) == 2:
            ...
            return await ctx.send(["ページ数は整数で1~3で指定してね!", "Specify the number of pages as an integer from 1 to 3!", "페이지 수는 정수 1 ~ 3 중에서 지정주세요!", "¡Especifique el número de páginas como un número entero de 1 a 3!"])
    ...
    async def list_head(self, ctx):
        ...
        if len(listed) == 2:
            ...
            return await ctx.send(["ページ数は整数で1~6で指定してね!", "Specify the number of pages as an integer from 1 to 6!", "페이지 수는 정수 1 ~ 6 중에서 지정주세요!", "¡Especifique el número de páginas como un número entero de 1 a 6!"])
    ...
    async def list_body(self, ctx):
        ...
        if len(listed) == 2:
            ...
            return await ctx.send(["ページ数は整数で1~7で指定してね!", "Specify the number of pages as an integer from 1 to 7!", "페이지 수는 정수 1 ~ 7 중에서 지정주세요!", "¡Especifique el número de páginas como un número entero de 1 a 7!"])
